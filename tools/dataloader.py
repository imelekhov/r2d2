...
from tools.augmentations import init_transforms
...
class PairLoader:
    ...
    def __init__(self, dataset, crop='', scale='', distort='', norm=norm_RGB,
                 what='aflow mask', do_color_aug=False, idx_as_rng_seed=False):
    ...
    def __getitem__(self, i):
        ...
        is_correct_prob = False if abs(sum(prob) - 1.) > np.sqrt(np.finfo(prob.dtype).eps) else True
        ...
        for _ in range(50 * self.n_samples):
            ...
            '''
            min_Q_y, max_Q_y = Q(y2[valid], win2[0]).min(), Q(y2[valid], win2[0]).max()
            min_Q_x, max_Q_x = Q(x2[valid], win2[1]).min(), Q(x2[valid], win2[1]).max()
            cond = False
        ...
        if self.DO_COLOR_AUG:
            img1, img2 = self.aug(image=img_a)["image"], self.aug(image=img_b)["image"]
        else:
